...
def get_cipher_score(text,cipher,scoring_params):
    ...
    score=score_params_on_cipher(decrypt_text)
    cipher_score=0
    for i,j in score.items():
        if i in scoring_params:
            cipher_score+=j*math.log(scoring_params[i])
    return cipher_score
